...
#                   with the arm-asm-254 backend (hand-tuned ARM
#                   assembly) for BN254 — this is the ASM reference build.
#                   A separate build-relic-bn254-easy-O2.sh builds the same
#                   curve with ARITH=easy (pure C) for the apples-to-apples
#                   comparison vs AmorE.
#
#  Output: <project root>/relic_install_bn254_asm/lib/librelic_s.a
#          <project root>/relic_install_bn254_asm/include/relic/*.h
#
#  Key properties:
...
RELIC_BUILD="${PROJECT_ROOT}/relic_build_bn254_asm"
RELIC_INSTALL="${PROJECT_ROOT}/relic_install_bn254_asm"
...
echo "  ARITH:    arm-asm-254  (hand-tuned ARM assembly)"
...
EXTRA_FLAGS="-O3 -funroll-loops -fomit-frame-pointer -ffunction-sections -fdata-sections -fno-strict-aliasing"
...
if grep -q "Selected ARITH backend.*arm-asm-254" "${LOGS}/relic_configure.log" 2>/dev/null \
   || grep -q "ARITH=arm-asm-254" "${LOGS}/relic_configure.log"; then
