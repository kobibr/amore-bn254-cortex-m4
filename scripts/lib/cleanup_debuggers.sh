...
# Helper (module-private): conditional logger
__cleanup_log() {
    if [ "${__CLEANUP_VERBOSE:-true}" = "true" ]; then
        echo "  [cleanup] $*"
    fi
}

cleanup_debuggers() {
    __CLEANUP_VERBOSE="${1:-true}"
    ...
    for pname in openocd gdb-multiarch arm-none-eabi-gdb; do
        ...
        if [ -n "$pids" ]; then
            __cleanup_log "Found $pname zombies: $pids - killing politely"
    ...
    for pname in openocd gdb-multiarch arm-none-eabi-gdb; do
        pids=$(pgrep -f "^$pname" 2>/dev/null || true)
        if [ -n "$pids" ]; then
            __cleanup_log "Force killing surviving $pname: $pids"
    ...
    if pgrep -f "^(openocd|gdb-multiarch|arm-none-eabi-gdb)" >/dev/null 2>&1; then
        __cleanup_log "ERROR: Debugger processes still alive after cleanup"
    ...
    if ! lsusb 2>/dev/null | grep -qiE "st-link|stlink|0483:374b"; then
        __cleanup_log "ERROR: ST-Link not found in USB. Check cable/power."
        return 1
    fi

    # Step 7: Verify /dev/ttyACM* exist (UART)
    if ! ls /dev/ttyACM* >/dev/null 2>&1; then
        __cleanup_log "WARNING: No /dev/ttyACM* devices found"
        # Not fatal - flash via ST-Link works regardless
    fi

    __cleanup_log "ST-Link clean and ready"
    return 0
}
